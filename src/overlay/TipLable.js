...
function TipLable(options) {
    ...
    var myIcon = icon ?
        new BMap.Icon(icon.url, new BMap.Size(icon.size.width, icon.size.height), {    //小车图片
        //offset: new BMap.Size(0, -5),    //相当于CSS精灵
        imageOffset: new BMap.Size(0, 0)  //图片的偏移量。为了是图片底部中心对准坐标点。
        }) : {};
    var marker = this.marker = icon ?
        new BMap.Marker(this.point, {icon: myIcon}) :
        new BMap.Marker(this.point);
